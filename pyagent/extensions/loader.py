...
from ..user_runtime import resolve_user_dir, user_skills_dir
...
def collect_skill_text(agent: Any) -> str:
    ...
    Reads ``agent._this_skills`` (keys declared this turn at
    `input`/`before_agent_start`/`turn_start`, or rotated in from last turn's
    `turn_end` declarations) and appends each matching
    ``~/.pyagent/skills/extensions/<key>.md``. A total char budget guards
    against bloat; missing/unreadable files are skipped (fault isolation).
    Called from ``Agent._refresh_system_message`` every turn/iteration.
    """
    keys = getattr(agent, "_this_skills", None)
    if not keys:
        return ""
    skills_dir = user_skills_dir(resolve_user_dir(agent.config.user_dir)) / "extensions"
    chunks: list[str] = []
    used = 0
