...
from .project_context import AvailableSkill, list_available_skills, resolve_available_skill
...
def _format_skill_list(skills: list[AvailableSkill], scope: str) -> list[str]:
    scoped = [skill for skill in skills if skill.scope == scope]
    heading = "User skills (`~/.pyagent/skills/`):" if scope == "user" else "Project skills:"
    lines = [heading]
    if not scoped:
    ...
    for skill in scoped:
        ...
        lines.append(f"- `{skill.id}` — {title}")
        lines.append(f"  path: `{skill.label}`")
        if skill.preview:
...
def list_skills(scope: str = "all", config: AppConfig | None = None, cwd: str | None = None) -> str:
    """List user and project skills available for on-demand loading."""
    normalized_scope = (scope or "all").strip().lower()
    if normalized_scope not in {"all", "user", "project"}:
        return "Error: scope must be one of: all, user, project."

    runtime_config = config or AppConfig.from_env()
    base_cwd = cwd or os.getcwd()
    skills = list_available_skills(base_cwd, user_dir=runtime_config.user_dir)
    lines = [
        "Available skills:",
        "Skills are plain-text guidance files. Use `load_skills` with one or more skill IDs to return their contents as tool context; this does not change the system prompt.",
        "",
    ]
    if normalized_scope in {"all", "user"}:
        lines.extend(_format_skill_list(skills, "user"))
    if normalized_scope == "all":
        lines.append("")
    if normalized_scope in {"all", "project"}:
        lines.extend(_format_skill_list(skills, "project"))
    return _truncate("\n".join(lines))
...
def _normalize_skill_arguments(
    ...
    if isinstance(skills, str):
        requested.append(skills)
    elif isinstance(skills, list):
        requested.extend(str(item) for item in skills if str(item).strip())
...
def load_skills(
    ...
    """Return the contents of one or more skills as tool output."""
    requested = _normalize_skill_arguments(skill=skill, skills=skills)
    if not requested:
        return "Error: provide `skill` or `skills` using IDs returned by list_skills."

    runtime_config = config or AppConfig.from_env()
    base_cwd = cwd or os.getcwd()
    blocks = [
        "Loaded skills as tool context only. The system prompt and session-loaded skill list were not changed."
    ]
    ...
    for skill_id in requested:
        ...
        if resolved is None:
            blocks.append(
                f"## {skill_id}\n\nError: skill not found. Use `list_skills` and pass a returned skill ID."
            )
            continue
        if resolved.error:
            blocks.append(f"## {resolved.id}\n\nError reading skill: {resolved.error}")
            continue
        try:
            content = resolved.path.read_text(encoding="utf-8").strip()
        except OSError as exc:
            blocks.append(f"## {resolved.id}\n\nError reading skill: {exc}")
            continue
        ...
        blocks.append(
            ...
            f"Path: `{resolved.label}`\n\n"
            f"{content if content else '[empty skill file]'}"
        )
...
def _default_builtin_specs(config: AppConfig) -> list[ToolSpec]:
    ...
    return [
        ...
        ToolSpec(
            name="list_skills",
            description=(
                "List available user and project skill files. Skills are plain-text guidance that can be "
                "loaded on demand with load_skills without changing the system prompt."
            ),
            parameters={
                "type": "object",
                "properties": {
                    "scope": {
                        "type": "string",
                        "description": "Which skills to list: all, user, or project.",
                        "enum": ["all", "user", "project"],
                        "default": "all",
                    },
                },
            },
            handler=lambda scope="all": list_skills(scope=scope, config=config),
        ),
        ToolSpec(
            name="load_skills",
            description=(
                "Load one or more skill files by returning their contents as tool output. This does not "
                "persistently add the skill to the system prompt or session context. Use skill IDs from list_skills."
            ),
            parameters={
                "type": "object",
                "properties": {
                    "skill": {
                        "type": "string",
                        "description": "A single skill ID from list_skills, such as project:skills/tools.md.",
                    },
                    "skills": {
                        "type": "array",
                        "description": "One or more skill IDs from list_skills.",
                        "items": {"type": "string"},
                    },
                },
            },
            handler=lambda skill=None, skills=None: load_skills(
                skill=skill,
                skills=skills,
                config=config,
            ),
